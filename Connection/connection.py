...
class Connection:
    def __init__(self, message_bus, control_bus, port, listener, service_name, *args, **kwargs):
    ...
    def connect_to_queue(self, host, port, listener):
        ...
        while i < 3:
            try:
                conn.connect("admin", "admin", wait=True, headers={"client-id": os.environ['HOSTNAME']})
                break
            except Exception:
                print("connection to %s:%d failed. Retrying in 5 seconds" % (host, port))
                time.sleep(5)
            i += 1
